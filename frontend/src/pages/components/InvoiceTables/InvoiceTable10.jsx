...
import { Trash2, Plus, Save, X, DollarSign, TrendingUp, Zap } from "lucide-react";
    ...
    <div className="space-y-6">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {draftInvoices.map(invoice => (
                <tr key={invoice.id} className="hover:bg-gray-50 transition-colors duration-150">
                  {["category_identifier", "source_doc", "Datum", "Omschrijving", "kWh", "Bedrag", "BTW"].map(field => (
                    <td
                      key={field}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="flex items-center p-4 bg-white shadow rounded-lg">
          ...
          <div>
            <p className="text-sm text-gray-500">Total kWh</p>
            <p className="text-lg font-semibold text-gray-900">{totalKWH.toFixed(2)}</p>
          </div>
        </div>
        {/* Total excl */}
        <div className="flex items-center p-4 bg-white shadow rounded-lg">
