...
import { Trash2, Plus, Save, X, DollarSign, TrendingUp, Factory, Eye } from "lucide-react";
...
export default function InvoiceTable({ categoryIdentifier }) {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const data = res.data.data.map(inv => ({
        ...
        MKK: +(Number(inv.Bedrag || 0)).toFixed(2)
      }));
      setInvoices(data);
      ...
      prev.map(inv => {
        if (inv.id === id) {
          ...
          if (field === "Bedrag" || field === "BTW") {
            ...
            updated.MKK = +(amountNum).toFixed(2);
          }
          return updated;
        }
        return inv;
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = draftInvoices.map(({ MKK, ...rest }) => rest);
  ...
  const totalMK = draftInvoices.reduce((sum, inv) => sum + Number(inv.MKK || 0), 0);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {draftInvoices.map(invoice => (
                <tr key={invoice.id} className="hover:bg-gray-50 transition-colors duration-150">
                  {["category_identifier", "Datum", "Omschrijving", "Bedrag", "BTW"].map(field => (
                    <td
                      key={field}
                  ...
                  <td className="px-3 py-2 text-right font-semibold">€{invoice.MKK.toFixed(2)}</td>
