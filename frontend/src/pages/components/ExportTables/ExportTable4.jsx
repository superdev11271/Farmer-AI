...
const TotalsTable = memo(({ sub_index, item_index, sub_name, categoryIdentifier, setExportJsonArray }) => {
    const [totals, setTotals] = useState({ totalMK: 0, totalJV: 0, totalMV: 0, totalZK: 0, totalExcl: 0, totalIncl: 0, Verschil: 0 });
    ...
    const fetchTotals = async () => {
        try {
            ...
            const Verschil = res.data.length > 0 ? res.data.reduce((sum, inv) => ((Number(inv.mk || 0) + Number(inv.jv || 0) + Number(inv.mv || 0) + Number(inv.zk || 0)) - Number(inv.bedrag || 0))) : 0;
