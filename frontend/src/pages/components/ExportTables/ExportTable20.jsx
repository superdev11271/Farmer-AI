...
const TotalsTable = memo(({ sub_index, item_index, sub_name, categoryIdentifier, setExportJsonArray }) => {
    ...
    const fetchTotals = async () => {
        try {
            ...
            const totalExcl = Object.values(tmp).reduce((sum, val) => sum + Number(val || 0), 0)

            setTotals({ totalExcl });
            setExportJsonArray(prev => [...prev, { table_name: `${sub_index}${item_index ? " - " + item_index : ""}: ${sub_name}`, data: { "TOTAAL BETEELDE OPPERVLAKTE": totalExcl } }])
        } catch (err) {
        ...
        <div className="card p-6 shadow-md rounded-lg bg-white flex justify-center">
            <div className="w-full max-w-xl">
                ...
                <table className="w-full border-collapse border border-gray-300 text-center">
                    ...
                    <tbody>
                        <tr>
                            <td className="border border-gray-300 px-4 py-2 text-center">{totals.totalExcl.toFixed(2)} ha</td>
                        </tr>
                    </tbody>
                </table>
            </div>
